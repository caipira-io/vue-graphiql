    ...
    <div class="relative">
        <button
            ...
            :class="isRunning
                ...
                : 'bg-[var(--gql-primary)] hover:brightness-110 text-white'"
            @click="handleClick"
        >
            <!-- Play icon -->
            <svg v-if="!isRunning" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-5 h-5 ml-0.5">
        ...
        <div
            ...
            class="absolute top-full left-1/2 -translate-x-1/2 mt-1 z-10 bg-[var(--gql-bg)] border border-[var(--gql-border)] rounded-lg shadow-xl py-1 min-w-40"
        >
            <button
                v-for="op in store.operations.value"
                :key="op.name?.value ?? 'anonymous'"
                class="w-full px-3 py-1.5 text-left text-sm text-[var(--gql-text)] hover:bg-[var(--gql-hover)] transition-colors"
